...
class POSViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        title = "Ventas Super"
        navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "barcode.viewfinder"), style: .plain, target: self, action: #selector(scanBarcode))
    ...
    @objc private func scanBarcode() {
        let scanner = BarcodeScannerViewController { [weak self] code in
            ...
            if let p = self.productos.first(where: { ($0["codigo"] as? String) == code }) {
                if let idx = self.cart.firstIndex(where: { ($0.producto["id"] as? Int) == (p["id"] as? Int) }) {
                    self.cart[idx].cantidad += 1
                } else { self.cart.append((p, 1)) }
                self.updateTotal(); self.cartTable.reloadData()
            } else {
                self.showAlert("No encontrado", "Código: \(code)")
            }
        }

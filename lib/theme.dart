...
class Tema {
  ...
  static const textDark = Color(0xFF232d29);
  static const textSoft = Color(0xFF5c6660);
  static const textMuted = Color(0xFF8fa096);
  ...
  static BoxDecoration darkCardDeco = BoxDecoration(
    color: darkCardBg,
    borderRadius: BorderRadius.circular(radius),
    border: Border.all(color: darkCardBorder, width: 1),
    boxShadow: [darkShadowSm],
  );

  static BoxDecoration cardDecoFor(bool dark) => dark ? darkCardDeco : cardDeco;
  ...
  static Widget kpiCard(String title, String value, IconData icon, {required Color accent, Color? bgTint}) {
    ...
    return Container(
      decoration: BoxDecoration(
        color: bgTint ?? (dark ? darkCardBg : Colors.white),
        borderRadius: BorderRadius.circular(radius),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text(title, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: dark ? darkTextSecondary : textSoft, letterSpacing: 0.3)),
          Icon(icon, color: accent.withValues(alpha: 0.7), size: 16),
        ]),
        const SizedBox(height: 4),
        Flexible(child: Text(value, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: dark ? darkTextPrimary : textDark, letterSpacing: -0.5), maxLines: 1, overflow: TextOverflow.ellipsis)),
      ]),
    );
  }
  ...
  static Widget sectionTitle(String text) => Padding(
    ...
    child: Text(text, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: isDark ? darkTextPrimary : textDark)),
  );

  static ThemeData theme = ThemeData(
    primaryColor: primary,
    scaffoldBackgroundColor: bg,
    colorScheme: ColorScheme.fromSeed(seedColor: primary, surface: cardBg, brightness: Brightness.light),
    fontFamily: 'Inter',
    ...
    cardTheme: CardThemeData(
      color: cardBg,
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(radius), side: const BorderSide(color: cardBorder, width: 1)),
    ...
    drawerTheme: const DrawerThemeData(backgroundColor: cardBg),
    ...
    dialogTheme: DialogThemeData(
      ...
      insetPadding: const EdgeInsets.fromLTRB(12, 16, 12, 24),
    ),
    ...
    bottomNavigationBarTheme: const BottomNavigationBarThemeData(backgroundColor: cardBg, selectedItemColor: primary, unselectedItemColor: textMuted, elevation: 0),
  );
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        hintStyle: const TextStyle(color: Tema.textMuted),
        prefixIcon: _focused
            ? null

...
class _RepScreenState extends State<RepScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> get _tabNames => const ['Ventas', 'Ganancias', 'Inventario', 'Fiados', 'Caja', 'Distribuciones', 'Ganancia Neta'];
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Desde', style: TextStyle(fontSize: 10, color: Tema.textMuted)),
                Text(_fmt(_desde), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textDark)),
              ]),
            ]),
          ),
        ),
      ),
      const Padding(padding: EdgeInsets.symmetric(horizontal: 4), child: Icon(Icons.arrow_forward, size: 16, color: Tema.textMuted)),
      Expanded(
        child: InkWell(
          onTap: _pickHasta,
          child: Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 6),
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Hasta', style: TextStyle(fontSize: 10, color: Tema.textMuted)),
                Text(_fmt(_hasta), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textDark)),
              ]),
            ]),
          ),
        ),
        ...
        if (dias.isNotEmpty) ...[
          Padding(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Ventas por Dia', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
              Text('${dias.length} dias', style: const TextStyle(fontSize: 11, color: Tema.textMuted)),
            ]),
          ),
        ...
        if (vf.isEmpty)
          ...
          Container(
            ...
            child: SingleChildScrollView(
              ...
              child: DataTable(
                columnSpacing: 16,
                headingTextStyle: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: Tema.textSoft),
                dataTextStyle: const TextStyle(fontSize: 12, color: Tema.textDark),
                columns: const [
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        if (chartData.isNotEmpty) ...[
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
            child: const Text('Ganancia por Dia', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
        ...
        if (dias.isEmpty)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
            child: Text('Resumen por dia', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
          Container(
            decoration: Tema.cardDeco,
            child: SingleChildScrollView(
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        if (deudores.isEmpty)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
            child: Text('Deudores', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
          ...deudores.map((d) {
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(d['nombre'], style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Tema.textDark)),
                      const SizedBox(height: 2),
                      Text('$activos fia.', style: const TextStyle(fontSize: 11, color: Tema.textSoft)),
                    ]),
                  ),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text(Fb.formatMoney(total), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Tema.danger)),
                    const Text('pendiente', style: TextStyle(fontSize: 10, color: Tema.textMuted)),
                  ]),
                ]),
              ),
            );
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        if (byTipo.isNotEmpty) ...[
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
            child: const Text('Resumen por Tipo', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
            ...
            child: Column(children: byTipo.entries.map((e) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(e.key, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textDark))),
        ...
        if (cf.isEmpty)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
            child: Text('Cajas (${cf.length})', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
          ...cf.map((c) {
            final ing = _cajaIng(c);
            final egr = _cajaEgr(c);
            ...
            return Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('Caja #${c['id']}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
                  Container(
  ...
  Widget _statMini(String label, String value, Color color) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(fontSize: 10, color: Tema.textMuted)),
      const SizedBox(height: 2),
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        if (dis.isEmpty)
          ...
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
            child: Text('Historial (${dis.length})', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
          ...dis.map((d) {
            ...
            return Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text(fecha, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Tema.textDark)),
                ...
                if (cats.isNotEmpty) ...[
                  ...
                  ...cats.map((c) => Padding(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: Text((c['nombre'] ?? '').toString(), style: const TextStyle(fontSize: 12, color: Tema.textDark))),
                      Text('${((c['porcentaje'] as num?)?.toDouble() ?? 0).toStringAsFixed(1)}%', style: const TextStyle(fontSize: 11, color: Tema.textMuted)),
                      const SizedBox(width: 12),
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        if (chartData.isNotEmpty) ...[
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
            child: const Text('Ganancia Neta por Mes', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: Tema.textDark)),
          ),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: Text(
                          ...
                          style: const TextStyle(fontSize: 8, color: Tema.textMuted),
                          textAlign: TextAlign.center,
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
  ...
  Widget _empty(String msg) {
    return Container(
      ...
      decoration: Tema.cardDeco,
      child: Column(children: [
        const Icon(Icons.inbox, size: 40, color: Tema.textMuted),
        const SizedBox(height: 8),
        Text(msg, style: const TextStyle(fontSize: 13, color: Tema.textMuted)),
      ]),
    );
  }
}

...
class _CprScreenState extends State<CprScreen> {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) => AlertDialog(
          ...
          title: Text('Abastecer: ${template['nombre'] ?? ''}', style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark)),
          content: SizedBox(
            width: double.maxFinite,
            child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Proveedor: ${template['proveedor_nombre'] ?? ''}', style: const TextStyle(color: Tema.textSoft)),
              ...
              const Text('Productos:', style: TextStyle(fontWeight: FontWeight.w600, color: Tema.textDark)),
              const SizedBox(height: 4),
              ...productos.map((p) => Padding(
                padding: const EdgeInsets.symmetric(vertical: 3),
                child: Row(children: [
                  Expanded(child: Text(p['nombre']?.toString() ?? '', style: const TextStyle(fontSize: 13))),
                  Text('x${p['cantidad'] ?? 1}', style: const TextStyle(fontWeight: FontWeight.w600)),
                  const SizedBox(width: 12),
                  Text(Fb.formatMoney(p['subtotal'] ?? 0), style: const TextStyle(fontWeight: FontWeight.w600, color: Tema.primary)),
                ]),
              )),
              const Divider(height: 16),
              Row(children: [
                const Text('Total estimado: ', style: TextStyle(fontWeight: FontWeight.w600)),
                Text(Fb.formatMoney(totalEstimado), style: const TextStyle(fontWeight: FontWeight.w800, color: Tema.primary, fontSize: 16)),
              ]),
            ]))),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancelar')),
            ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Confirmar Abastecimiento')),
          ],
        ),
      ),
    );
  ...
  Future<void> _eliminarTemplate(Map<dynamic, dynamic> template) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(Tema.radiusLg)),
        title: const Text('Eliminar Plantilla', style: TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark)),
        content: Text('Eliminar plantilla "${template['nombre'] ?? ''}"?\nEsta accion no se puede deshacer.', style: const TextStyle(color: Tema.textSoft)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancelar')),
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(nombre, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: Tema.textDark)),
                  const SizedBox(height: 2),
                  Text(proveedorNombre, style: const TextStyle(fontSize: 12, color: Tema.textSoft)),
                ]),
              ),
            ...
            Row(children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text('Historial (${historial.length})', style: const TextStyle(fontSize: 12)),
                  style: OutlinedButton.styleFrom(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                    side: const BorderSide(color: Tema.cardBorder),
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Abastecer', style: TextStyle(fontSize: 12)),
  ...
  Widget _infoChip(IconData icon, String text) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(text, style: const TextStyle(fontSize: 11, color: Tema.textSoft)),
    ]);
  }
}
        ...
        if (_searchResults.isNotEmpty) ...[
          ...
          ..._searchResults.map((prod) => Container(
            ...
            child: Row(children: [
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(prod['nombre']?.toString() ?? '', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Tema.textDark)),
                  Text('Cod: ${prod['codigo'] ?? '-'} | Stock: ${prod['stock_actual'] ?? prod['stock'] ?? 0}', style: const TextStyle(fontSize: 11, color: Tema.textSoft)),
                ]),
              ),
              ElevatedButton.icon(
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 6),
          const Text('Productos de la Plantilla', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Tema.textDark)),
          const Spacer(),
        ...
        if (_items.isEmpty)
          ...
          ...List.generate(_items.length, (idx) {
            ...
            return Container(
              ...
              child: Row(children: [
                Expanded(
                  child: Text(nombre, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Tema.textDark)),
                ),
                Container(
                  decoration: BoxDecoration(
                    color: Tema.cardBg,
...
class _QuickProvDialogState extends State<_QuickProvDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(Tema.radiusLg)),
      insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24),
      title: Text('Nuevo Proveedor: ${widget.nombre}', style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark)),
      content: SizedBox(
        width: double.maxFinite,
        child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      actions: [
        ...
        ElevatedButton(onPressed: _guardar, child: const Text('Guardar Proveedor')),
      ],
    );
  }
}
      ...
      builder: (ctx, scrollCtl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text('Historial: $nombre', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 18, color: Tema.textDark)),
            const SizedBox(height: 4),
            Text('${historial.length} abastecimientos registrados', style: const TextStyle(color: Tema.textSoft, fontSize: 13)),
            const SizedBox(height: 16),
              ...
              ...historial.map((h) => Container(
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: const Icon(Icons.shopping_cart, color: Tema.primary, size: 20),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(_fmtDate(h['fecha']), style: const TextStyle(fontWeight: FontWeight.w600, color: Tema.textDark)),
                      Text('${h['items_count'] ?? 0} items', style: const TextStyle(fontSize: 12, color: Tema.textSoft)),
                    ]),
                  ),
                  Text(Fb.formatMoney(h['total'] ?? 0), style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.primary, fontSize: 15)),
                ]),
              )),
          ],
        ),

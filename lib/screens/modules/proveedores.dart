        ...
        builder: (ctx, setSt) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              _fld('Contacto', contactoPaxf),
              _fld('Telefono', telPaxf),
              _fld('Email', emailPaxf),
                ...
                Expanded(
                  child: OutlinedButton(
                    ...
                    style: OutlinedButton.styleFrom(
                      ...
                      side: const BorderSide(color: Tema.textMuted),
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(Tema.radiusSm),
                      ),
                    ),
                    child: const Text('Cancelar', style: TextStyle(color: Tema.textDark, fontWeight: FontWeight.bold)),
                  ),
                ),
  ...
  Widget _fld(String l, TextEditingController ctl, {bool number = false}) {
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(labelText: l, border: const OutlineInputBorder(), isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
      ),
    );
  }
      ...
      body: Column(children: [
        ...
        Expanded(
          child: _mostrarCalendario
              ? _buildCalendario()
              : fl.isEmpty
                    ? ListView(children: const [Padding(padding: EdgeInsets.all(32), child: Text('No se encontraron proveedores', textAlign: TextAlign.center, style: TextStyle(color: Tema.textMuted)))])
                    : ListView.builder(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        itemCount: fl.length,
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(nombre, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: Tema.textDark)),
            ...
            if (contacto.isNotEmpty || telefono.isNotEmpty) ...[
              ...
              Flexible(child: Row(children: [
                ...
                if (contacto.isNotEmpty && telefono.isNotEmpty) const SizedBox(width: 12),
                if (telefono.isNotEmpty) Flexible(child: _infoChip(Icons.phone_outlined, telefono)),
              ])),
            ],
            const SizedBox(height: 8),
            Flexible(child: Row(children: [
              const Icon(Icons.history, size: 14, color: Tema.textMuted),
              const SizedBox(width: 4),
              Flexible(child: Text(ultVisita, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, color: Tema.textSoft))),
              const SizedBox(width: 8),
...
class _ProveedorDetalle extends StatelessWidget {
      ...
      builder: (ctx, scrollCtl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 16),
              Row(children: [
               ...
               Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                 Text(nombre, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 18, color: Tema.textDark)),
            ...
            if (visitas.isEmpty)
              const Padding(padding: EdgeInsets.all(16), child: Text('Sin visitas registradas', style: TextStyle(color: Tema.textMuted)))
            else
              ...visitas.take(10).map((v) => Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text((v['observaciones'] ?? '').toString().isEmpty ? 'Sin observaciones' : (v['observaciones'] ?? '').toString(), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Tema.textSoft)),
                ),
              )),
            const SizedBox(height: 8),
            _seccionTitulo('Ultimas Compras'),
            if (comprasReales.isEmpty)
              const Padding(padding: EdgeInsets.all(16), child: Text('Sin compras registradas', style: TextStyle(color: Tema.textMuted)))
            else
              ...comprasReales.take(10).map((c) => Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(_fmtDate(c['fecha']), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Tema.textSoft)),
  ...
  Widget _detalleRow(IconData icon, String label, String value) {
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 10),
        Flexible(child: Text('$label: ', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textSoft))),
        Expanded(child: Text(value, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, color: Tema.textDark))),
      ]),
    );
  }
  ...
  Widget _seccionTitulo(String text) {
    return Padding(
      ...
      child: Text(text, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Tema.textDark)),
    );
  }

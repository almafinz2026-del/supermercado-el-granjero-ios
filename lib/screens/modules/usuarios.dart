...
class _UsuScreenState extends State<UsuScreen> with SingleTickerProviderStateMixin {
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) {
          return AlertDialog(
            ...
            title: Text(edit ? 'Editar Usuario' : 'Nuevo Usuario', style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark)),
            content: SizedBox(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                TextField(
                  controller: pw,
                  obscureText: true,
                  decoration: InputDecoration(labelText: edit ? 'Contrasena (dejar vacio)' : 'Contrasena *', border: const OutlineInputBorder(), isDense: true, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
                ),
                const SizedBox(height: 10),
      ...
      builder: (dialogCtx) => StatefulBuilder(
        builder: (dialogCtx, setSt) {
          return AlertDialog(
            ...
            title: Text('Permisos de $tipo: $nombre', style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark, fontSize: 16)),
            content: SizedBox(
              width: double.maxFinite,
  ...
  Future<void> _addRole() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24),
        title: const Text('Nuevo Rol', style: TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark)),
        content: SizedBox(
          width: double.maxFinite,
          child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    if (_usuarios.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          const Text('No hay usuarios registrados', style: TextStyle(color: Tema.textMuted, fontSize: 15)),
        ]),
      );
    }
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark, fontSize: 15),
                      ),
                      ...
                      Row(
                        children: [
                          Text(
                            '@$username',
                            style: const TextStyle(fontSize: 12, color: Tema.textSoft),
                          ),
                          const SizedBox(width: 8),
                          Container(
                      ...
                      if (lastAccessStr.isNotEmpty) ...[
                        ...
                        Row(
                          children: [
                            ...
                            Text(lastAccessStr, style: const TextStyle(fontSize: 10, color: Tema.textMuted)),
                          ],
                        ),
                      ],
                    ],
    ...
    if (_roles.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          const Text('No hay roles definidos', style: TextStyle(color: Tema.textMuted, fontSize: 15)),
        ]),
      );
    }
    return ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          return Container(
            ...
            child: ListTile(
              ...
              title: Text(r['nombre']?.toString() ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700, color: Tema.textDark)),
              subtitle: Text(
                '${r['descripcion']?.toString() ?? 'Sin descripcion'}  \u2014  $count usuario${count != 1 ? 's' : ''}',
                maxLines: 1, overflow: TextOverflow.ellipsis,
                style: const TextStyle(color: Tema.textSoft, fontSize: 12),
              ),
            ...
            ? Center(
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text(_acciones.isEmpty ? 'Sin acciones registradas' : 'Sin resultados',
                      style: const TextStyle(color: Tema.textMuted)),
                ]),
              )
            : ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  return Container(
                    ...
                    child: ListTile(
                      ...
                      title: Text(a['accion']?.toString() ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Tema.textDark)),
                      subtitle: Text(
                        '${a['usuario'] ?? a['usuario_nombre'] ?? '-'}  \u2014  $fecha',
                        maxLines: 1, overflow: TextOverflow.ellipsis,
                        style: const TextStyle(color: Tema.textSoft, fontSize: 11),
                      ),
                      trailing: a['detalle'] != null
                          ? Container(
                              ...
                              child: Text(a['detalle'].toString(), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11, color: Tema.textMuted)),
                            )
                          : null,
                    ),
                  );

...
class _FacScreenState extends State<FacScreen> {
  ...
  void _addToCartCustom(Map<dynamic,dynamic> prod) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Precio personalizado', style: TextStyle(color: Tema.textDark, fontWeight: FontWeight.w700)),
        content: TextField(
          controller: pcC,
    ...
    await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) {
          ...
          return AlertDialog(
            title: const Text('Distribuir Ganancia', style: TextStyle(color: Tema.textDark, fontWeight: FontWeight.w700)),
              ...
              child: SingleChildScrollView(
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  if (preview.isNotEmpty) ...[
                    ...
                    const Text('Distribucion proporcional:', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: Tema.textDark)),
  ...
  @override Widget build(BuildContext c) {
    ...
    final children = <Widget>[
      ...
      Padding(padding: const EdgeInsets.fromLTRB(10,10,10,4), child: TextField(controller: _clientC, decoration: InputDecoration(hintText: 'Nombre del cliente', prefixIcon: const Icon(Icons.person_outline, size: 20), fillColor: Tema.cardBg, filled: true, contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), border: OutlineInputBorder(borderRadius: BorderRadius.circular(Tema.radiusSm), borderSide: BorderSide.none)), style: const TextStyle(fontSize: 14))),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), child: TextField(controller: _searchC, focusNode: _focusSearch, decoration: InputDecoration(hintText: 'Buscar producto...', prefixIcon: _focusedSearch ? null : const Icon(Icons.search, size: 20), suffixIcon: _searchF.isNotEmpty ? IconButton(icon: const Icon(Icons.clear, size: 18), onPressed: () { _searchC.clear(); setState(() => _searchF = ''); }) : null, fillColor: Tema.cardBg, filled: true, contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), border: OutlineInputBorder(borderRadius: BorderRadius.circular(Tema.radiusSm), borderSide: BorderSide.none)), onChanged: (v) => setState(() => _searchF = v), style: const TextStyle(fontSize: 14))),
      Expanded(flex: 3, child: GridView.builder(padding: const EdgeInsets.all(8), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 1.0, crossAxisSpacing: 8, mainAxisSpacing: 8), itemCount: prods.length, itemBuilder: (_, i) {
        final p = prods[i];
        ...
        return GestureDetector(
          ...
          child: Container(decoration: Tema.cardDeco, padding: const EdgeInsets.all(12), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Text('${p['nombre']}', textAlign: TextAlign.center, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textDark)),
    ...
    if (_cart.isNotEmpty) {
      ...
      children.add(Container(decoration: BoxDecoration(color: Tema.cardBg, borderRadius: const BorderRadius.vertical(top: Radius.circular(Tema.radius)), boxShadow: [Tema.shadowSm]), padding: const EdgeInsets.all(10), child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          Expanded(child: Text('${_cart.length} items', style: const TextStyle(fontSize: 12, color: Tema.textSoft))),
          Row(children: [
            ...
            Text(_descuentoPct ? '%' : '\$', style: const TextStyle(fontSize: 12, color: Tema.textSoft)),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Total:', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Tema.textDark)),
          Text(Fb.formatMoney(_total), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Tema.primary)),
        ]),
        if (_totalCostos > 0)
          Padding(
            padding: const EdgeInsets.only(top: 2),
            child: Text('Costos adicionales: +${Fb.formatMoney(_totalCostos)}', style: const TextStyle(fontSize: 11, color: Tema.textSoft)),
          ),
        const SizedBox(height: 6),
        InkWell(
        ...
        if (_showCostos) ...[
          ...
          if (_totalCostos > 0)
            Padding(
              ...
              child: Text('Total costos: ${Fb.formatMoney(_totalCostos)}', style: const TextStyle(fontSize: 11, color: Tema.textSoft)),
            ),
        ],
        const SizedBox(height: 6),
        SizedBox(
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          ElevatedButton(onPressed: _checkout, style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10)), child: Text(_pagado ? 'Cobrar' : 'Guardar Factura')),
        ]),
      ])));

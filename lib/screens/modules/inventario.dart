      ...
      await showDialog<bool>(
        ...
        builder: (ctx) => StatefulBuilder(
          builder: (ctx, setSt) {
            ...
            return AlertDialog(
          title: Text(edit ? 'Editar Producto' : 'Nuevo Producto',
              style: const TextStyle(color: Tema.textDark, fontWeight: FontWeight.w700)),
          insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24),
          content: SizedBox(
            width: double.maxFinite,
            child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              _fld('Codigo', cdC, required: true),
              _fld('Nombre', nmC, required: true),
              Padding(
              ...
              _fld('Precio Compra', pcC, num: true),
              _fld('Precio Venta', pvC, num: true),
              _fld('Stock Actual', stC, num: true),
              _fld('Stock Minimo', smC, num: true),
              const SizedBox(height: 4),
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      if (edit) {
                        final idx = _p.indexWhere((x) => x['id'] == prod['id']);
                        if (idx >= 0) _p[idx] = {..._p[idx], ...data};
                      } else {
                        data['id'] = _nextId();
                        _p.add(data);
                      }
                      // Force UI update immediately, don't wait for Firestore stream
                      if (mounted) setState(() {});
          ...
          child: Row(children: [
            ...
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                decoration: InputDecoration(
                  hintText: 'Categoria',
                  hintStyle: const TextStyle(color: Tema.textMuted, fontSize: 12),
                  filled: true,
    ...
    return SingleChildScrollView(
      ...
      child: SingleChildScrollView(
        child: DataTable(
          ...
          dataRowMaxHeight: 52,
          headingRowHeight: 42,
          headingTextStyle: const TextStyle(fontWeight: FontWeight.w700, fontSize: 10, color: Tema.textDark),
          dataTextStyle: const TextStyle(fontSize: 10, color: Tema.textSoft),
          columns: const [
    ...
    await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) {
          ...
          return AlertDialog(
            title: const Text('Descuentos Masivos', style: TextStyle(color: Tema.textDark, fontWeight: FontWeight.w700)),
            insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 24),
            content: SizedBox(
              width: double.maxFinite,
              child: SingleChildScrollView(

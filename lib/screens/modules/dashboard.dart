        ...
        children: [
          CircleAvatar(
            ...
            child: photoProvider == null
                ? Text(initial, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white))
                : null,
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Flexible(
                      child: Text(displayName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                    ),
                    if (_userRole != null) ...[
                      ...
                      Container(
                        ...
                        child: Text(_userRole!, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.white)),
                      ),
                    ],
                  ],
                ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(label, style: const TextStyle(fontSize: 10, color: Tema.textSoft)),
        ],
      ),
    );
  }
  ...
  Widget _buildLowStock() {
    return Column(
      ...
      children: [
        Row(
          children: [
            const Icon(Icons.warning_amber_rounded, color: Tema.danger, size: 20),
            const SizedBox(width: 8),
            const Text('Stock Bajo', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Tema.danger)),
            const Spacer(),
            Text('${_stockBajo.length} productos', style: const TextStyle(fontSize: 12, color: Tema.textMuted)),
          ],
        ),
        const SizedBox(height: 8),
        Container(
            ...
            children: _stockBajo.take(5).map((p) {
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: Text(nombre, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textDark)),
                    ),
                    Container(
                      ...
                      child: Text(
                        '$sa / $sm',
                        style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Tema.danger),
                      ),
                    ),
                  ],
                ),
          ...
          child: Column(
            children: datos.asMap().entries.map((e) {
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    SizedBox(width: 32, child: Text(d['dia'] as String, style: const TextStyle(fontSize: 11, color: Tema.textMuted))),
                    ...
                    const SizedBox(width: 8),
                    SizedBox(width: 50, child: Text(Fb.formatMoney(total), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Tema.textDark))),
                  ],
                ),
              );
            }).toList(),
                        ...
                        children: [
                          ...
                          FractionallySizedBox(
                            ...
                            child: Container(
                              ...
                              child: Text(nombre, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.white)),
                            ),
                          ),
                        ],
                      ),
  ...
  Widget _buildRecentSales() {
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            const Icon(Icons.receipt_long_rounded, color: Tema.primary, size: 20),
            const SizedBox(width: 8),
            const Text('Ventas Recientes', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Tema.textDark)),
          ],
        ),
        const SizedBox(height: 8),
        Container(
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.receipt_long_rounded, color: Tema.primary, size: 18),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(cliente, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Tema.textDark)),
                          const SizedBox(height: 2),
                          Text('$fecha  ·  $items items', style: const TextStyle(fontSize: 11, color: Tema.textMuted)),
                        ],
                      ),
                    ),
                    Text(
                      Fb.formatMoney(total),
                      style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Tema.primary),
                    ),
                  ],
                ),
              );
